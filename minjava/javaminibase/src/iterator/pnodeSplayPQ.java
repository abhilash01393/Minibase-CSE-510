...
public class pnodeSplayPQ extends pnodePQ {
  ...
  */
	...
	public void enqPref(pnode item,AttrType[] t1, AttrType[] t2,short len_in, short[] str_sizes, int[] pref_list, int pref_list_length) throws IOException, UnknowAttrType, TupleUtilsException {
		count++;
		pnodeSplayNode newnode = new pnodeSplayNode(item);
		pnodeSplayNode t = root;
		...
		int comp = pnodeCMPPref(item, t.item,t1,t2,len_in,str_sizes,pref_list,pref_list_length);
		...
		while (!done) {
			if ((sort_order.tupleOrder == TupleOrder.Ascending && comp >= 0) || (sort_order.tupleOrder == TupleOrder.Descending && comp <= 0)) {
				...
				} else comp = pnodeCMPPref(item, tr.item,t1,t2,len_in,str_sizes,pref_list,pref_list_length);
				...
				if ((sort_order.tupleOrder == TupleOrder.Ascending && comp <= 0) || (sort_order.tupleOrder == TupleOrder.Descending && comp >= 0)) {
					...
					} else comp = pnodeCMPPref(item, trr.item,t1,t2,len_in,str_sizes,pref_list,pref_list_length);
				...
				} else comp = pnodeCMPPref(item, tl.item,t1,t2,len_in,str_sizes,pref_list,pref_list_length);
				...
				if ((sort_order.tupleOrder == TupleOrder.Ascending && comp >= 0) || (sort_order.tupleOrder == TupleOrder.Descending && comp <= 0)) {
					...
					} else comp = pnodeCMPPref(item, tll.item,t1,t2,len_in,str_sizes,pref_list,pref_list_length);
