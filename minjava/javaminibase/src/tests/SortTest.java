...
class SORTDriver extends TestDriver
  ...
  protected boolean test5() {
    ...
    // Create unsorted data file "test5.in"
    RID rid;
    Heapfile f = null;
    try {
    ...
    for (int i = 0; i < 20; i++) {
      // setting fields
      inum1 = random1.nextFloat();
      inum2 = random2.nextFloat();
    ...
    // Sort "test3.in" on the int attribute (field 3) -- Ascending
    System.out.println(" -- Sorting in ascending order on the two int fields -- ");

    try {
      fscan = new FileScan("test5.in", attrType, attrSize, (short) 2, 2, projlist, null);
    } catch (Exception e) {
      status = FAIL;
      e.printStackTrace();
    }
    ...
    SortPref sort = null;
    try {
      sort = new SortPref(attrType, (short) 2, attrSize, fscan, order[0], pref_list, 2, SORTPGNUM);
    } catch (Exception e) {
      status = FAIL;
      e.printStackTrace();
    }
    ...
    while (t != null) {
      ...
      try {
        latestsum = 0;
        latestsum =  t.getFloFld(1);
        latestsum = latestsum + t.getFloFld(2);
      ...
      count++;
      if (latestsum < lastsum) {
        System.err.println("count = " + count + " latest = " + latestsum + " last = " + lastsum);
        System.err.println("Test5 -- OOPS! test5.out not sorted");
        status = FAIL;
        break;
      }
      lastsum = latestsum;
    ...
    if (count < 20) {
      ...
      System.err.println("Test5 -- Sorting of pref_list fields OK\n");
    }
